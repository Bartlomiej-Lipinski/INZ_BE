...
public class UpdateUserTwoFactorVerificationStatus : IEndpoint
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation("Attempting to update two-factor status for user ID: {UserId}. TraceId: {TraceId}", userId, traceId);

        var user = await dbContext.Users.FindAsync(new object[] { userId }, cancellationToken);
        if (user == null)
        {
            logger.LogWarning("User not found for two-factor status update with ID: {UserId}. TraceId: {TraceId}", userId, traceId);
            return Results.NotFound(ApiResponse<string>.Fail("User not found.", traceId));
        }

        if (user.TwoFactorEnabled == flag)
        {
            logger.LogWarning("No change in two-factor verification status for user ID: {UserId}. TraceId: {TraceId}", userId, traceId);
            return Results.BadRequest(ApiResponse<string>.Fail("Two-factor verification status is already set to the specified value.", traceId));
        }
        ...
        if (updated > 0)
        {
            logger.LogInformation("Two-factor verification status successfully updated for ID: {UserId} to {Status}. TraceId: {TraceId}", 
                userId, flag, traceId);
            return Results.Ok(ApiResponse<string>.Ok("Two-factor verification status updated successfully.", null, traceId));
        }
